...
describe('User Controller', () => {
    describe('boundary', () => {
        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access /create protected routes without a token`, async () => {
            const response = await request(app).get('/create');
            expect(response.status).toBe(401);
        });
        
        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access get /:id protected routes without a token`, async () => {
            const response = await request(app).get('/1234');
            expect(response.status).toBe(401);
        });
        
        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access put /:id protected routes without a token`, async () => {
            const response = await request(app).put('/create');
            expect(response.status).toBe(401);
        });
        
        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access delete /:id protected routes without a token`, async () => {
            const response = await request(app).delete('/:id');
            expect(response.status).toBe(401);
        });
        
        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access /:id/activity protected routes without a token`, async () => {
            const response = await request(app).get('/1234/activity');
            expect(response.status).toBe(401);
        });

        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access /:id/favorites protected routes without a token`, async () => {
            const response = await request(app).get('/1234/favorites');
            expect(response.status).toBe(401);
        });

        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access put /change-password/:id protected routes without a token`, async () => {
            const response = await request(app).get('/change-password/1234');
            expect(response.status).toBe(401);
        });

        it(`${userControllerBoundaryTest} should return 401 Unauthorized when trying to access /email/:email protected routes without a token`, async () => {
            const response = await request(app).get('/email/email@mail.com');
            expect(response.status).toBe(401);
        });
